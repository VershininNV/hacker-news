...
PWA на Angular - Hacker News. Отображение самых последних новостей с главной страницы [портала](https://news.ycombinator.com/)
...
# Развёртывание проекета

В проекте настроено авторазвёртывание через GitHub Pages и GitHub Actions. Проект доступен по: [ссылке](https://vershininnv.github.io/hacker-news/). Либо Вы можете развернуть проект локально. Для этого следуйте шагам ниже.
# 1. Скачивание проекта

Скопируйте в командную строку, либо воспользуйтесь ПО для работы с репозиториями.

```bash
  git clone https://github.com/VershininNV/hacker-news.git
```


# 2.1. Запуск проекта с Docker
...
```bash
  docker run -d -p 8080:80 hacker-news
```
